...
coco_dir = '/work/u6144480/coco'
save_dir = 'temp/coco'
if not os.path.exists(save_dir):
    os.makedirs(save_dir)

data = json.load(open(os.path.join(coco_dir, 'annotations/instances_train2017.json')))
categories = []
catId2catName = {}
...
for line in data['annotations']:
    ...
    img_path = os.path.join(coco_dir, 'train2017', img_name)
...
data = json.load(open(os.path.join(coco_dir, 'annotations/instances_val2017.json')))
catId2catName = {}
for line in data['categories']:
    catId2catName[line['id']] = line['name']
imgId2imgName = {}
...
for line in data['annotations']:
    ...
    img_id = line['image_id']
    img_name = imgId2imgName[img_id]
    img_path = os.path.join(coco_dir, 'val2017', img_name)
    if cat_name not in imgName2catName[img_path]:
        imgName2catName[img_path].append(cat_name)
imgName2catName = ['{}\t{}\n'.format(k, ','.join(v)) for k, v in imgName2catName.items()]
...
#===========================================================================================
data = json.load(open(os.path.join(coco_dir, 'annotations/image_info_test-dev2017.json')))
catId2catName = {}
for line in data['categories']:
    catId2catName[line['id']] = line['name']
imgId2imgName = {}
...
for line in data['annotations']:
    ...
    img_id = line['image_id']
    img_name = imgId2imgName[img_id]
    img_path = os.path.join(coco_dir, 'test2017', img_name)
    if cat_name not in imgName2catName[img_path]:
        imgName2catName[img_path].append(cat_name)
