...
class EarlyStopping:
    ...
    def __init__(self, patience, subclass, warmup=0, deno=300, verbose=False, alpha=0.9, sub_beta = None, ignore=[], path='loss.png', trace_func=print, max_iter=None, fixed_threshold=False):
        """
        Args:
    ...
    def __call__(self, subclass_loss, iter_loss, model):
        ...
        if start:
            ...
            self.class_average_loss.append(class_average_loss/len(self.subclass_loss.keys()))
        else:
            return
        #================================================
        ...
        if not self.previous_convergence_state:
            ...
            if len(self.class_average_loss)==1:
                ...
                if self.fixed_threshold:
                    threshold = 0.0
                    for sub in self.subclass:
                        if sub in self.ignore: continue
        ...
        if (self.early_stop == True or self.iteration%500==0) and len(self.class_average_loss)>1:
            ...
            #     plt.close('all')
            plot_3D(self.plot_3D, self.x_axis, self.color_3D)
            plot_2D(self.plot_3D, self.x_axis, self.color_3D)
            # np.save('class_average_loss.npy', np.array(self.iter_arr))

    def AllConverged(self):
        all_converged = True
        for sub in self.subclass:
            if sub in self.ignore:
                self.subclass_converge[sub] = True
                continue
